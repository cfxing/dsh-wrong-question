...
export class WrongQuestionDb {
    ...
    `);
        ...
        try {
            this.db.exec('ALTER TABLE questions ADD COLUMN analysis TEXT');
        }
        catch { }
        try {
            this.db.exec('ALTER TABLE questions ADD COLUMN image_data TEXT');
        }
        catch { }
        try {
            this.db.exec("ALTER TABLE questions ADD COLUMN artifacts TEXT NOT NULL DEFAULT '[]'");
        }
        catch { }
    }
    close() {
    ...
    `).run(id, input.content, input.answer ?? old?.answer ?? '', input.source ?? old?.source ?? null, input.imagePath ?? old?.imagePath ?? null, input.imageData ?? old?.imageData ?? null, json(input.artifacts ?? old?.artifacts ?? []), input.ocrText ?? old?.ocrText ?? null, json(input.knowledgePoints ?? old?.knowledgePoints ?? []), json(input.tags ?? old?.tags ?? []), Math.min(5, Math.max(1, Number(input.difficulty ?? old?.difficulty ?? 3))), input.mistakeCause ?? old?.mistakeCause ?? null, input.analysis ?? old?.analysis ?? null, input.followupQuestion ?? old?.followupQuestion ?? null, created, now, review.reps, review.ease, review.intervalDays, review.dueAt, review.lastReviewedAt ?? null);
        this.reindex(id);
    ...
    list({ limit = 20, offset = 0, tag, knowledgePoint, dueOnly = false } = {}) {
        const where = [];
        const args = [];
        if (tag) {
        ...
        if (knowledgePoint) {
            where.push('knowledge_points LIKE ?');
            args.push(`%"${knowledgePoint}"%`);
        }
        if (dueOnly) {
            where.push('due_at <= ?');
    ...
    search(query, limit = 10) {
        ...
        try {
            ...
            for (const r of rows)
                found.set(r.id, { question: parseQuestion(r), score: 1 / (1 + Math.max(0, Number(r.rank))), matchType: 'fts' });
        }
        catch { }
        if (found.size < limit) {
            ...
            for (const r of rows)
                if (!found.has(r.id))
                    found.set(r.id, { question: parseQuestion(r), score: .5, matchType: 'fts' });
        }
    ...
    findSimilar(id, limit = 10) {
        ...
        return this.all().filter(q => q.id !== id).map(question => {
            ...
            return { question, score: Number((kp * .5 + tags * .25 + text * .25).toFixed(4)), matchType: 'similarity' };
    ...
    logs(limit = 1000) {
        return this.db.prepare('SELECT * FROM review_logs ORDER BY reviewed_at DESC LIMIT ?').all(limit).map((r) => ({
    ...
    dashboard() {
        ...
        for (const q of qs)
            for (const p of q.knowledgePoints) {
                const x = kp.get(p) ?? { count: 0, reviewed: 0, interval: 0 };
                x.count++;
                if (q.review.reps > 0)
                    x.reviewed++;
                x.interval += Math.min(1, q.review.intervalDays / 30);
                kp.set(p, x);
            }
        ...
        for (const l of logs)
            activityMap.set(l.reviewedAt.slice(0, 10), (activityMap.get(l.reviewedAt.slice(0, 10)) ?? 0) + 1);
        let d = new Date(now);
        d.setHours(0, 0, 0, 0);
        let streak = 0;
        while (activityMap.has(d.toISOString().slice(0, 10))) {
            streak++;
            d.setDate(d.getDate() - 1);
        }
        const success = logs.filter(l => l.quality >= 3).length;
        const days = lastDays(30, now), daily = new Map();
        for (const l of logs) {
            const date = l.reviewedAt.slice(0, 10);
            const xs = daily.get(date) ?? [];
            xs.push(l);
            daily.set(date, xs);
        }
        ...
        for (const q of qs) {
            const cause = (q.mistakeCause ?? '未分类').trim() || '未分类';
            mistakeMap.set(cause, (mistakeMap.get(cause) ?? 0) + 1);
        }
    ...
    graph() {
        ...
        for (const q of this.all()) {
            ...
            for (const p of ps) {
                const x = nodes.get(p) ?? { count: 0, mastery: 0, due: 0 };
                x.count++;
                x.mastery += Math.min(100, Math.round((Math.min(1, q.review.reps / 3) * .7 + Math.min(1, q.review.intervalDays / 30) * .3) * 100));
                if (isDue(q.review))
                    x.due++;
                nodes.set(p, x);
            }
...
function searchTokens(value) { return value.normalize('NFKC').match(/[\p{L}\p{N}]+/gu)?.filter(Boolean).slice(0, 20) ?? []; }
function parseArtifacts(value) { try {
    const xs = JSON.parse(String(value || '[]'));
    return Array.isArray(xs) ? xs.filter(x => x && ['image', 'video', 'html'].includes(x.kind)) : [];
}
catch {
    return [];
} }
function terms(value) {
    ...
    const chunks = normalized.match(/[a-z0-9]+|[\p{Script=Han}]+/gu) ?? [], words = [];
...
function jaccard(a, b) { const x = new Set(a.map(v => v.trim().toLocaleLowerCase()).filter(Boolean)), y = new Set(b.map(v => v.trim().toLocaleLowerCase()).filter(Boolean)); if (!x.size && !y.size)
    return 0; let common = 0; for (const v of x)
    if (y.has(v))
        common++; return common / (x.size + y.size - common); }
function lastDays(count, now) { const out = []; for (let i = count - 1; i >= 0; i--) {
